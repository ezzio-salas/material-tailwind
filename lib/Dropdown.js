...
export default function Dropdown({
    ...
    }else if (buttonType === 'outlinewhite') {
        classes.push(...buttonOutlineWhite);
    }else if (buttonType === 'linkwhite') {
        classes.push(...buttonLinkWhite);
    }
     else {
     ...
     return (
         <div>
            <Tippy
                content={
                    <div
                        className={`bg-white text-base z-50 float-left list-none text-left rounded-lg shadow-lg mt-1 p-2 overflow-hidden transition-all duration-500  overflow-y-scroll`}
                        style={{ minWidth: '10rem', ,maxHeight:"50vh"}} }}
                    >
                        {children}
                    </div>
                }
            ...
            >
                <button
                    {...rest}
                    className={`${classes} ${className} ${buttonHeader !== undefined ?'':'justify-center'}`}
                    type="button"
                ...
                >
                    { buttonHeader !== undefined ? <div className="flex flex-col ">
                 <p className="text-sm text-gray-300 ">{buttonHeader}</p> 
                 <p className="justify-center">  {buttonText}</p>
                 </div>       :<p className="justify-center">  {buttonText}</p>
